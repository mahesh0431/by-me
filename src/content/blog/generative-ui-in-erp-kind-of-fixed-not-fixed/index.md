---
title: "Generative UI in ERP: Dynamic, But Deterministic"
slug: generative-ui-in-erp-kind-of-fixed-not-fixed
created: 2026-04-25
updated: 2026-04-26
tags:
  - genai
...
> - Generative UI is getting faster and more impressive, but latency still matters a lot. Even a few seconds can feel like YouTube buffering when the user expects an interface instantly.
...
So recently I was playing with a small browser experiment around Generative UI. It was inspired by Ben Cobley's Flash Lite Browser idea, and I made a Codex Spark version of it for fun using Codex App Server.

The surprising part was not that it worked. The surprising part was that it felt fast, and still I got irritated waiting.

The page started rendering in around two seconds and completed in maybe three or four seconds. Compared to where Generative UI demos were even a year back, this is quite fast. Models are getting extremely faster, and you can feel that shift when you play with these small experiments.

But still, that small wait gave me the same feeling I get when YouTube buffers.

<video controls muted playsinline preload="metadata" src="/blog/generative-ui-in-erp-kind-of-fixed-not-fixed/codex-spark-browser.mp4">
  Codex Spark browser experiment video.
</video>

That is when this thought started: maybe Generative UI is not just a question of whether the model can create the interface. Maybe it is also a question of whether the user can tolerate the wait.
...
When we talk about generative UI, it is very easy to get excited about the output. You type something, the model generates a screen, and suddenly the browser feels like it can become anything.
...
When I think about SAP, [Fiori elements](https://www.sap.com/design-system/fiori-design-web/v1-96/discover/frameworks/sap-fiori-elements/smart-templates) is already an interesting example. It is not Generative UI in the full AI sense, but it is also not a fully hand-built static UI. The template responds to metadata from the backend API and shows the UI based on that.

So in one sense, it is already generative UI, but template-based and API-driven.

And honestly, that direction works quite well.

That is why I do not think the future of ERP Generative UI is pure free-form generation. There has to be some kind of template, metadata, or validation structure. In CAP/OData terms, [annotations](https://cap.cloud.sap/docs/guides/uis/fiori#fiori-annotations) are already one way of carrying that UI-relevant meaning with the service model.

Maybe the intelligent model understands the metadata and dynamically creates the experience around it. Maybe Fiori elements, or something in that direction, evolves into a more dynamic generative UI model.
...
In normal AI demos, hallucination is already a problem. In ERP UI, it is more serious because the generative interface can influence user action.
...
For something like enterprise Generative UI, the system probably needs an agentic loop. It has to check, iterate, validate, and ensure the outcome is proper.
...
It felt much faster than older Generative UI demos, but still not fast enough for that instant UI feeling.
...
<video controls muted playsinline preload="metadata" src="/blog/generative-ui-in-erp-kind-of-fixed-not-fixed/flipbook-live.mp4">
...
## So where does this leave Generative UI?

For now, I think Generative UI is still not very practical for serious enterprise workflows.
...
So maybe we are still in the 3G era of Generative UI.

Everything is technically possible, but you can still feel the buffering.

At some point, the 5G moment may come, where there is no buffering, no waiting, and the generative interface feels smooth and natural.
...
Generative UI, but with templates.

Generative UI, but based on metadata.

Generative UI, but with backend validations.

Generative UI, but deterministic where it matters.

Generative UI that is kind of fixed, not fixed.
...
So that's it folks, let me know your thoughts. I am also still exploring how practical this whole Generative UI direction is for enterprise systems, so any corrections, alternate views, or better examples are much appreciated.
